...
;
$query_ConsultaChange = sprintf("SELECT * FROM cotizaciones WHERE cotizaciones.id = %s", GetSQLValueString($varID_ConsultaChange, "int"));
...
<?php
//Actualizar Estado
  if ($row_ConsultaChange["urgente"] == '1') { 
  $updateSQL = sprintf("UPDATE cotizaciones SET cotizaciones.urgente = '0' WHERE cotizaciones.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  }
  if ($row_ConsultaChange["urgente"] == '0') { 
  $updateSQL = sprintf("UPDATE cotizaciones SET cotizaciones.urgente = '1' WHERE cotizaciones.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  }
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "cotizaciones_urgentes.php";
