...
<?php
//Actualizar Estado
  if ($row_ConsultaChange["vista"] == '1') { 
  $updateSQL = sprintf("UPDATE cotizaciones SET cotizaciones.vista = '0' WHERE cotizaciones.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  }
  if ($row_ConsultaChange["vista"] == '0') { 
  $updateSQL = sprintf("UPDATE cotizaciones SET cotizaciones.vista = '1' WHERE cotizaciones.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  }
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "cotizaciones_vistas.php";
