...
;
$query_ConsultaChange = sprintf("SELECT * FROM contacto WHERE contacto.id = %s", GetSQLValueString($varID_ConsultaChange, "int"));
...
<?php
//Actualizar Estado
  $updateSQL = sprintf("UPDATE contacto SET contacto.importante = '0' WHERE contacto.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "contactos_importantes.php";
