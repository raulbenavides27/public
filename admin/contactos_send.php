...
;
$query_mensajeSend = "SELECT * FROM contacto WHERE contacto.estado = '2' ORDER BY contacto.id DESC";
$mensajeSend = mysqli_query($DKKadmin, $query_mensajeSend) or die(mysqli_error($DKKadmin));
$row_mensajeSend = mysqli_fetch_assoc($mensajeSend);
$totalRows_mensajeSend = mysqli_num_rows($mensajeSend);
?>
<?
$destinatario = "{$row_mensajeSend['correo']}";
$asunto = "Hola {$row_mensajeSend['nombre']}, te hemos enviado un mensaje nuevo.";
...
<div class='wrapper'>
    <div class='cuerpo'>
        <div class='contenido'>
            ...
            <!-- contenido -->
            <div class='mensaje'>
            	<h3>Hola  <b>{$row_mensajeSend['nombre']}</b></h3>
                <p>Te estamos escribiendo desde prodalum.cl:</p>
                <p>
                <blockquote>{$row_mensajeSend['mensaje']}</blockquote><br />
              </p>
...
$headers .= "Reply-To: {$row_mensajeSend['correo']}\r\n";
...
?>
<META HTTP-EQUIV='Refresh' CONTENT='0; URL=contactos_enviados.php'>
<?php
mysqli_free_result($datosAdmin);

mysqli_free_result($mensajeSend);
?>
