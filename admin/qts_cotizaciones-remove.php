...
$query_ConsultaChange = sprintf("SELECT * FROM qts_cotizaciones WHERE qts_cotizaciones.id = %s", GetSQLValueString($varID_ConsultaChange, "int"));
...
//Actualizar Estado
  $updateSQL = sprintf("UPDATE qts_cotizaciones SET qts_cotizaciones.estado = '0' WHERE qts_cotizaciones.id = %s",GetSQLValueString($varID_ConsultaChange, "int"));
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "qts_clientes-view.php?id=".$row_ConsultaChange["clienteID"];
