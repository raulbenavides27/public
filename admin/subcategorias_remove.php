...
$query_ConsultaChange = sprintf("SELECT * FROM menuSubCategorias WHERE menuSubCategorias.id = '$idCategoria'");
...
<?php
//Actualizar Estado
  $updateSQL = sprintf("UPDATE menuSubCategorias SET menuSubCategorias.estado = '0' WHERE menuSubCategorias.id = '$idCategoria'");
  mysqli_select_db($DKKadmin, $database_DKKadmin);
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "subcategorias.php";
