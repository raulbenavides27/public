...
$contactosPendientes = mysqli_query($DKKadmin, $query_contactosPendientes);
...
$query_qtsCotizacionEditar = sprintf("SELECT * FROM qts_cotizaciones WHERE qts_cotizaciones.id = '$cotizacionID'");
...
$estadoCotizacion = $row_qtsCotizacionEditar["estado"];
...
$idModelo = $row_qtsProductosCotizacion["idModelo"];

$query_modeloSeleccionado = "SELECT * FROM productosTabla WHERE productosTabla.id = '$idModelo'";
$modeloSeleccionado = mysqli_query($DKKadmin, $query_modeloSeleccionado);
...
$query_motivosAccion = "SELECT * FROM cotizacionesMotivos WHERE cotizacionesMotivos.estado = '1' AND cotizacionesMotivos.estadoCotizacion = '$estadoCotizacion' ORDER BY cotizacionesMotivos.id ASC";
$motivosAccion = mysqli_query($DKKadmin, $query_motivosAccion);
$row_motivosAccion = mysqli_fetch_assoc($motivosAccion);
$totalRows_motivosAccion = mysqli_num_rows($motivosAccion);
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editarProductoForm")) {
  ...
  $updateGoTo = "qts_productos_edit.php?id=".$_POST['id'];
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "nuevoProductoForm")) {
  $insertSQL = sprintf("INSERT INTO qts_cotizacionesProductos (idProducto, idCliente, idCreador, idCotizacion, qty, precio, statusDscto, dscto, fecha, fechaID, ip, estado) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($_POST['estado'], "text"));
  $Result1 = mysqli_query($DKKadmin, $insertSQL) or die(mysqli_error($DKKadmin));
  $insertGoTo = "qts_productos_edit.php?id=".$idCotizacion;
  if (isset($_SERVER['QUERY_STRING'])) {
    $insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
    $insertGoTo .= $_SERVER['QUERY_STRING'];
  }
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "observacionesForm")) {
  $updateSQL = sprintf("UPDATE qts_cotizaciones SET motivo=%s WHERE id=%s",
                       GetSQLValueString($_POST['motivo'], "text"),
                       GetSQLValueString($_POST['id'], "int"));
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "qts_edit.php?id=".$idCotizacion;
  header(sprintf("Location: %s", $updateGoTo));
}

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "totalForm")) {
  $updateSQL = sprintf("UPDATE qts_cotizaciones SET total=%s WHERE id=%s",
                       GetSQLValueString($_POST['total'], "text"),
                       GetSQLValueString($_POST['id'], "int"));
  $Result1 = mysqli_query($DKKadmin, $updateSQL) or die(mysqli_error($DKKadmin));
  $updateGoTo = "qts_enviar.php?id=".$idCotizacion;
  header(sprintf("Location: %s", $updateGoTo));
}

$ivaCotizacion = ($row_subTotal['SUM(qts_cotizacionesProductos.total)'] * '0.19');
$totalCotizacion = round($row_subTotal['SUM(qts_cotizacionesProductos.total)'] + $ivaCotizacion);
?>
<!DOCTYPE html>
        ...
        <div id="page-container" class="sidebar-l sidebar-o side-scroll header-navbar-fixed">
            ...
            <main id="main-container">
				...
				<?php if ($row_datosAdmin["nivel"] != '1') { ?>
                	<?php if ($row_qtsCotizacionEditar["creadorID"] != $row_datosAdmin["id"]) { ?>
                    ...
                    <div class="row items-push">
                        <div class="col-sm-7">
                            <h1 class="page-heading">
                                Cotización <small>Editar Cotización</small>
                            </h1>
                        </div>
                        <div class="col-sm-5 text-right hidden-xs">
                            <ol class="breadcrumb push-10-t">
                                <li>Dashboard</li>
                                <li><a class="link-effect" href="">Editar Cotización</a></li>
                            </ol>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="block-header">
                            <ul class="block-options">
                                <li>
									...
									<script>
                                        function qte<?php echo $row_qtsCotizacionEditar["id"]; ?>()
                                        {
                                            self.name = 'opener';
                                            remote = open('../qte/qte.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>', 'remote', 'width=612,height=802,location=no,scrollbar=no,menubars=no,toolbars=no,resizable=no,fullscreen=no, status=yes');
    	                                    remote.focus();
	                                    }	
                                    </script>
                                    <button type="button" onclick="location.href='javascript:qte<?php echo $row_qtsCotizacionEditar["id"]; ?>();'"><i class="si si-printer"></i> Ver PDF</button>
                                </li>
                                <li>
                            ...
                            <div class="row">
                            	<div class="col-xs-6 text-left">
                                    ...
                                    <address class="font-s12">
                                        ...
                                        <b>Fecha de Emisión:</b> <?php echo date("d/m/Y",$row_qtsCotizacionEditar["fechaID"]); ?>
                                    </address>
                                    <span class="hidden-print label label-<?php if ($row_qtsCotizacionEditar["estado"] == '1') { echo "warning"; } if ($row_qtsCotizacionEditar["estado"] == '2') { echo "primary"; } if ($row_qtsCotizacionEditar["estado"] == '3') { echo "success"; } if ($row_qtsCotizacionEditar["estado"] == '4') { echo "danger";} if ($row_qtsCotizacionEditar["estado"] == '5') { echo "default";} if ($row_qtsCotizacionEditar["estado"] == '6') { echo "danger";} if ($row_qtsCotizacionEditar["estado"] == '0') { echo "danger";} ?>"><?php if ($row_qtsCotizacionEditar["estado"] == '1') { echo "INGRESADA"; } if ($row_qtsCotizacionEditar["estado"] == '2') { echo "ENVIADA"; } if ($row_qtsCotizacionEditar["estado"] == '3') { echo "APROBADA"; } if ($row_qtsCotizacionEditar["estado"] == '4') { echo "RECHAZADA";} if ($row_qtsCotizacionEditar["estado"] == '5') { echo "CERRADA";} if ($row_qtsCotizacionEditar["estado"] == '6') { echo "ELIMINADA";} if ($row_qtsCotizacionEditar["estado"] == '0') { echo "REMOVIDA";} ?></span>
                                </div>
                            	<div class="col-xs-6 text-right">
									...
									<?php if ($row_qtsCotizacionEditar["lugarEntrega"] == '3') { ?>
                                    <address class="font-s12">
                                    ...
                                    <address class="font-s12">
                                        <strong>Lugar de Entrega:</strong> <?php if ($row_qtsCotizacionEditar["lugarEntrega"] == '1') {echo"Retiro en PRODALUM Ltda.";} if ($row_qtsCotizacionEditar["lugarEntrega"] == '2') {echo"Por Coordinar";} if ($row_qtsCotizacionEditar["lugarEntrega"] == '3') {echo"Despacho";} ?><br>
                                        ...
                                        <strong>Método de Pago:</strong> <?php if ($row_qtsCotizacionEditar["metodoPago"] == '1') {echo"Efectivo / Transferencia";} if ($row_qtsCotizacionEditar["metodoPago"] == '2') {echo"Tarjeta de Débito";} if ($row_qtsCotizacionEditar["metodoPago"] == '3') {echo "Tarjeta de Crédito <small>".$row_qtsCotizacionEditar["metodoPagoAdicional"]."</small>";} ?><br>
                            ...
                            <div class="table-responsive">
                                        ...
                                        <div class="form-group">
                                            <div class="col-xs-12 push-10-t">
                                                <?php if ($totalRows_qtsProductosCotizacion > 0) { ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '1') { ?>
													<form name="totalForm" method="POST" action="<?php echo $editFormAction; ?>">
														<input type="hidden" name="total" value="<?php echo $totalCotizacion; ?>">
														<button class="btn btn-sm btn-warning" title="Añadir Observaciones" data-toggle="modal" data-target="#addObservaciones" type="button"><i class="fa fa-pencil"></i></button>
														<button class="btn btn-sm btn-primary" type="button" title="Añadir Producto" data-toggle="modal" data-target="#nuevoProducto"><i class="fa fa-plus"></i></button>
                                                		<button class="btn btn-sm btn-success" title="Enviar" type="submit"><i class="fa fa-send"></i></button>
														<input type="hidden" name="id" value="<?php echo $idCotizacion; ?>">
														<input type="hidden" name="MM_update" value="totalForm">
													</form>
													<?php } ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '2') { ?>
                                                	<button class="btn btn-sm btn-warning" title="Añadir Observaciones" data-toggle="modal" data-target="#addObservaciones" type="button"><i class="fa fa-pencil"></i></button>
                                                	<button class="btn btn-sm btn-success" title="Aprobar" type="button" onClick="location.href='qts_aprobar.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-check"></i></button>
                                                	<button class="btn btn-sm btn-danger" title="Rechazar" type="button" onClick="location.href='qts_rechazar.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-times"></i></button>
													<?php } ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '3') { ?>
                                                	<button class="btn btn-sm btn-warning" title="Añadir Observaciones" data-toggle="modal" data-target="#addObservaciones" type="button"><i class="fa fa-pencil"></i></button>
                                                	<button class="btn btn-sm btn-default" title="Cerrar" type="button" onClick="location.href='qts_cerrar.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-times"></i></button>
													<?php } ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '4') { ?>
                                                	<button class="btn btn-sm btn-warning" title="Añadir Observaciones" data-toggle="modal" data-target="#addObservaciones" type="button"><i class="fa fa-pencil"></i></button>
                                                	<button class="btn btn-sm btn-default" title="Cerrar" type="button" onClick="location.href='qts_cerrar.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-times"></i></button>
													<?php } ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '5') { ?>
                                                	<button class="btn btn-sm btn-warning" title="Añadir Observaciones" data-toggle="modal" data-target="#addObservaciones" type="button"><i class="fa fa-pencil"></i></button>
                                                	<button class="btn btn-sm btn-danger" title="Eliminar" type="button" onClick="location.href='qts_eliminar.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-trash"></i></button>
													<?php } ?>
													<?php if ($row_qtsCotizacionEditar["estado"] == '6') { ?>
                                                	<button class="btn btn-sm btn-danger" title="Remover" type="button" onClick="location.href='qts_remove.php?id=<?php echo $row_qtsCotizacionEditar["id"]; ?>'"><i class="fa fa-trash"></i></button>
													<?php } ?>
                                                <?php } ?>
                                            </div>
                                        </div>
                                        ...
                                        <table class="table table-bordered table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <p class="font-w600 push-10"><?php echo $pCotizados["nombre"]." Modelo: ".$row_modeloSeleccionado["modelo"]; ?> <span class="small font-s12 hidden-print"><a href="qts_productos_remove.php?id=<?php echo $row_qtsProductosCotizacion["id"]; ?>">(Eliminar)</a></span></p>
                                                        <div class="text-muted"><?php echo strtoupper(str_replace("-"," ",$pCotizados["categoriaSEO"])); ?></div>
                                                    </td>
                                                    <td class="text-right"><?php echo "$".number_format($row_qtsProductosCotizacion["precio"],0,",","."); ?></td>
                                ...
                                <div class="block block-bordered">
                                    ...
                                    <div class="block-content">
                                        <p><?php echo $row_qtsObservacionesCotizacion["observacion"]; ?></p>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                  <form method="POST" action="<?php echo $editFormAction; ?>" class="form-horizontal push-30-t push-30" name="observacionesForm">
									    ...
									    <div class="form-group">
                                            <div class="col-xs-12">
                                                <div class="form-material">
                                                    <select class="js-select2 form-control" id="motivo" name="motivo" style="width: 100%;" data-placeholder="El&iacute;gelo..">
                                                        <option></option>
                                                        <?php do { ?>
                                                        <option value="<?php echo $row_motivosAccion["id"]; ?>" <?php if ($row_motivosAccion["id"] == $row_qtsCotizacionEditar["motivo"]) {echo "selected";} ?>><?php echo $row_motivosAccion["motivo"]; ?></option>
                                                		<?php } while ($row_motivosAccion = mysqli_fetch_assoc($motivosAccion)); ?>
                                                    </select>
                                                    <label for="motivo">Motivo del Pendiente </label>
                                                </div>
                                            </div>
                                        </div>
									    <?php } ?>
									    ...
									    <div class="form-group">
                                            <div class="col-xs-12">
                                                <div class="form-material">
                                                    ...
                                                    <label for="motivo">Motivo de la Compra </label>
                                                </div>
                                            </div>
                                        </div>
									    <?php } ?>
                                        ...
                                        <input type="hidden" name="MM_update" value="observacionesForm">
                                    </form>
                                </div>
                            </div>
                        </div>
...
mysqli_free_result($motivosAccion);
