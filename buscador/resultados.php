...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "buscadorAvanzadoForm")) {
  $updateSQL = sprintf("UPDATE buscadorAvanzado SET peldanos=%s WHERE id=%s",
                       GetSQLValueString($_POST['peldanos'], "text"),
                       GetSQLValueString($_POST['id'], "int"));

  $Result1 = mysqli_query($DKKfront, $updateSQL) or die(mysqli_error($DKKfront));

  $updateGoTo = "resultados.php";
...
$query_busquedaAvanzadaIP = "SELECT * FROM buscadorAvanzado WHERE buscadorAvanzado.estado = '1' AND buscadorAvanzado.ip = '$ip' ORDER BY buscadorAvanzado.id DESC LIMIT 1";
$busquedaAvanzadaIP = mysqli_query($DKKfront, $query_busquedaAvanzadaIP);
$row_busquedaAvanzadaIP = mysqli_fetch_assoc($busquedaAvanzadaIP);
$totalRows_busquedaAvanzadaIP = mysqli_num_rows($busquedaAvanzadaIP);

//Variables de busqueda
$uso = $row_busquedaAvanzadaIP["uso"];
$capMax = $row_busquedaAvanzadaIP["capMax"];

$query_resultadosProductos = "SELECT * FROM productos WHERE productos.uso = '$uso' AND productos.capMax <= '$capMax' ORDER BY productos.id DESC";
$resultadosProductos = mysqli_query($DKKfront, $query_resultadosProductos);
$row_resultadosProductos = mysqli_fetch_assoc($resultadosProductos);
$totalRows_resultadosProductos = mysqli_num_rows($resultadosProductos);
...
<title><?php echo $row_metaDatos["titulo"]." | Buscador Avanzado | Resultados"; ?></title>
<!-- InstanceEndEditable -->
      ...
      <div class="row">
        <div class="col-main col-sm-9 col-xs-12">
          ...
          <div class="page-content checkout-page">
            <h4 class="checkout-sep">1. Selecciona su uso: Uso <b><?php if ($row_busquedaAvanzadaIP["uso"] == 'domestico') { echo "Dom&eacute;stico"; } if ($row_busquedaAvanzadaIP["uso"] == 'comercial') { echo "Comercial"; } if ($row_busquedaAvanzadaIP["uso"] == 'profesional') { echo "Profesional"; } ?></b></h4>
            
            <h4 class="checkout-sep">2. Selecciona su Capacidad M&aacute;xima: <b><?php echo $row_busquedaAvanzadaIP["capMax"]." kls."; ?></b></h4>
            
			<h4 class="checkout-sep">3. Altura M&aacute;xima: <b><?php echo $row_busquedaAvanzadaIP["alturaMax"]." mts."; ?></b></h4>
            
            <!--<h4 class="checkout-sep">4. Cantidad de Pelda&ntilde;os: <b><?php echo $row_busquedaAvanzadaIP["peldanos"]." pelda&ntilde;os"; ?></b></h4>-->
            
            <!-- resultados -->
            <h4 class="checkout-sep last">4. Resultados</h4>
            <div class="box-border">
              <div class="table-responsive">
                <table class="table table-bordered cart_summary">
                  <thead>
                    <tr>
                      <th class="cart_product">Escalera</th>
                      <th>Descripci&oacute;n</th>
                      <th>Stock</th>
                      <th class="action"><i class="fa fa-search"></i></th>
                    </tr>
                  </thead>
                  <tbody>
					<?php do { 
					$escalaID = $row_resultadosProductos["id"];
					$alturaMax = $row_busquedaAvanzadaIP["alturaMax"];
					?>
					<tr>
                    <?php $seleccionaProductosTabla = mysqli_query($DKKfront, "SELECT * FROM productosTabla WHERE productosTabla.estado = '1' AND productosTabla.escalaID = '$escalaID' AND productosTabla.alcanceMaximo <= '$alturaMax' ORDER BY productosTabla.id DESC LIMIT 1"); if(mysqli_num_rows($seleccionaProductosTabla) == 0) { echo ""; } else { ?>
                    
                    
					<?php $counter = 1; while($lSub = mysqli_fetch_array($seleccionaProductosTabla)) { ?>
                        <td class="cart_product"><a href="../productos/<?php echo $row_resultadosProductos["nombreSEO"]; ?>" target="_blank"><img src="../images/productos/<?php echo $row_resultadosProductos["imagen1"]; ?>" alt="<?php echo $row_resultadosProductos["nombre"]; ?>"></a></td>
                    	<td class="cart_description"><p class="product-name"><a href="../productos/<?php echo $row_resultadosProductos["nombreSEO"]; ?>" target="_blank"><?php echo $row_resultadosProductos["nombre"]; ?> </a></p>
                        <small style="text-transform: none;"><a href="../productos/<?php echo $row_resultadosProductos["nombreSEO"]; ?>" target="_blank"><strong>Modelo:</strong> <?php echo $lSub['modelo']; ?><br>
                        <strong>Pelda&ntilde;os:</strong> <?php echo $lSub['peldanos']." pelda&ntilde;os"; ?><br>
                        <strong>Capacidad M&aacute;x.:</strong> <?php echo $row_resultadosProductos["capMax"]." kls."; ?><br>
                        <strong>Alcance M&aacute;x.:</strong> <?php echo $lSub['alcanceMaximo']." mts."; ?></a></small></td>
                      	<td class="cart_avail"><span class="label label-success">En Stock</span></td>
                      	<td class="action"><a href="./productos/<?php echo $row_resultadosProductos["nombreSEO"]; ?>"><i class="fa fa-search"></i></a></td>
					<?php } ?>
                    <?php } ?>
                    </tr> 
                   
					<?php } while ($row_resultadosProductos = mysqli_fetch_assoc($resultadosProductos)); ?>
					  
                  </tbody>
                </table>
              </div>
              <button class="button pull-right" onClick="location.href='../buscador'"><span>Volver a Buscar</span></button>
            </div>
          </div>
        </div>
