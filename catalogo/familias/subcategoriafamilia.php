...
$pagina = $categoria.'/'.$subcategoria.'/'.$familia;
...
$query_familiaSelect = sprintf("SELECT * FROM menuSubCategoriasFamilias WHERE menuSubCategoriasFamilias.subCategoriaFamiliaSEO = '$familia' AND menuSubCategoriasFamilias.estado = '1'");
$familiaSelect = mysqli_query($DKKfront, $query_familiaSelect);
$row_familiaSelect = mysqli_fetch_assoc($familiaSelect);
$totalRows_familiaSelect = mysqli_num_rows($familiaSelect);

$familiaSEO = $row_familiaSelect["subCategoriaFamiliaSEO"];
...
$query_productosShow = "SELECT * FROM productos WHERE productos.estado = '1' AND productos.subCategoriaSEO = '$subcategoria' AND productos.subCategoriaFamiliaSEO = '$familia' ORDER BY productos.id DESC";
...
//actualizar visita
  $updateSQL = sprintf("UPDATE menuSubCategoriasFamilias SET menuSubCategoriasFamilias.visitas = menuSubCategoriasFamilias.visitas +1 WHERE menuSubCategoriasFamilias.subCategoriaSEO = '$subcategoria' AND menuSubCategoriasFamilias.subCategoriaFamiliaSEO = '$familia' AND menuSubCategoriasFamilias.estado = '1'");
...
<title><?php echo $row_metaDatos["titulo"]." | Catálogo de Productos | ".$row_categoriaSelect["categoria"]." | ".$row_subCategoriaSelect["subCategoria"]." | ".$row_familiaSelect["subCategoriaFamilia"]; ?></title>
<!-- InstanceEndEditable -->
...
if (screen.width<500) {
window.location="http://mobile.prodalum.cl/home/#!/../catalogo/familia.php?subcategoria=<?php echo $row_subCategoriaSelect["subCategoriaSEO"]; ?>&familia=<?php echo $row_familiaSelect["subCategoriaFamiliaSEO"]; ?>";
}
</script>-->
<!-- InstanceEndEditable -->
</head>
      ...
      <div class="row">
        <div class="col-xs-12">
          <ul>
            ...
            <li class=""> <a title="Ir a la SubCategoría" href="../<?php echo $row_categoriaSelect["categoriaSEO"]; ?>/<?php echo $row_subCategoriaSelect["subCategoriaSEO"]; ?>"><?php echo $row_subCategoriaSelect["subCategoria"]; ?></a><span>&raquo;</span></li>
            <li><strong><?php echo $row_familiaSelect["subCategoriaFamilia"]; ?></strong></li>
          </ul>
        </div>
      </div>
    </div>
        ...
        <div class="col-main col-sm-9 col-xs-12 col-sm-push-3">
          <div class="shop-inner">
            <div class="page-title">
              <h2><?php echo $row_familiaSelect["subCategoriaFamilia"]; ?></h2>
            </div>
            <div class="toolbar">
              <!-- <div class="view-mode">
                <ul>
				      ...
				      <div class="item-inner">
				        <div class="product-thumbnail">
				          <?php if (isset($row_productosShow["precioOferta"])) { ?>
				            ...
				            <?php } ?>
				          <?php if ($row_productosShow["novedad"] == '1') { ?><div class="icon-new-label new-right">Nuevo</div>
				          <?php } ?>
            ...
            <div class="pagination-area">
              ...
              <ul>
                <?php if ($pageNum_productosShow > 0) { // Show if not first page ?>
                  <li><a href="<?php printf("%s?pageNum_productosShow=%d%s", $currentPage, 0, $queryString_productosShow); ?>"><i class="fa fa-angle-double-left"></i></a></li>
                  <?php } // Show if not first page ?>
                <?php if ($pageNum_productosShow > 0) { // Show if not first page ?>
                  <li><a href="<?php printf("%s?pageNum_productosShow=%d%s", $currentPage, max(0, $pageNum_productosShow - 1), $queryString_productosShow); ?>"><i class="fa fa-angle-left"></i></a></li>
                  <?php } // Show if not first page ?>
                <?php if ($pageNum_productosShow < $totalPages_productosShow) { // Show if not last page ?>
                  <li><a href="<?php printf("%s?pageNum_productosShow=%d%s", $currentPage, min($totalPages_productosShow, $pageNum_productosShow + 1), $queryString_productosShow); ?>"><i class="fa fa-angle-right"></i></a></li>
                  <?php } // Show if not last page ?>
                <?php if ($pageNum_productosShow < $totalPages_productosShow) { // Show if not last page ?>
                  <li><a href="<?php printf("%s?pageNum_productosShow=%d%s", $currentPage, $totalPages_productosShow, $queryString_productosShow); ?>"><i class="fa fa-angle-double-right"></i></a></li>
                  <?php } // Show if not last page ?>
              </ul>
